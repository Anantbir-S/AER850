...
# Remove the initial fitting before CV
# my_model1.fit(x_train, y_train) <- Remove this
cv_scores_model1 = cross_val_score(LinearRegression(), x_train, y_train, cv=5, scoring='neg_mean_absolute_error')
...
# Remove the initial fitting before CV
# my_model2.fit(x_train, y_train) <- Remove this
cv_scores_model2 = cross_val_score(RandomForestRegressor(n_estimators=50, max_depth=5, min_samples_split=10, min_samples_leaf=5, max_features='sqrt', random_state=500953158), 
...
grid_search = GridSearchCV(my_model2_1, param_grid, cv=5, scoring='neg_mean_absolute_error', n_jobs=4)
...
y_pred_test1 = LinearRegression().fit(x_train, y_train).predict(x_test)  # Refitting the model for test set
