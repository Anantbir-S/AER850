...
#based on inspection of the corr_matrix, determine colinear variables and explore their correlations with y
...
#drop colinear variables that has lowe correlation with y
...
#checking that there is no correlation left
plt.figure()
corr_matrix_2=(X_train.iloc[:,0:-5]).corr()
sns.heatmap(np.abs(corr_matrix_2))
"""Training Model"""
my_model1 = LinearRegression()
#calling .fit finds the function that goes with all the variables
...
#.predict finds the y () values for a set of data, using the test set here
